...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "2379",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "3000",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    }, 
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "4000",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },        
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "TiDB8.5.1-HCE2.0" ? local.instance_image_id_maps_v1[data.huaweicloud_availability_zones.az.region] : (var.Version == "TiDB8.5.1-Ubuntu24.04" ? local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] )
